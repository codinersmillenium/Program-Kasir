  ...
  return $hasil_rupiah;
                      ...
                      if (isset($_GET['cari'])) {
                        ...
                        if ($tgl=="") {
                          ...
                          $x=mysql_query("SELECT tanggalmasuk,idproduk,namaproduk,sum(jumlahmasuk) as jumlah,sum(hargaawal*jumlahmasuk) as totalbeli FROM detbarangmasuk,barangmasuk,produk WHERE detbarangmasuk.kdbrgmsk=barangmasuk.idBMK and detbarangmasuk.idprodukm=produk.idproduk and tanggalmasuk='$tgl' group by idprodukm order by jumlah desc");
                            $data4 = mysql_fetch_array(mysql_query("SELECT tanggalmasuk,sum(jumlahmasuk) as jumlah FROM detbarangmasuk,barangmasuk where barangmasuk.idBMK=detbarangmasuk.kdbrgmsk and month(tanggalmasuk)='$tgl'"));
                            $jmlmasuk = $data4['jumlah'];
                        ...
                        if ($cekid > 0) {
                          ...
                          $data2 = mysql_fetch_array($q);
                          $x=mysql_query("SELECT tanggalmasuk,idproduk,namaproduk,sum(jumlahmasuk) as jumlah,sum(hargaawal*jumlahmasuk) as totalbeli FROM detbarangmasuk,barangmasuk,produk WHERE detbarangmasuk.kdbrgmsk=barangmasuk.idBMK and detbarangmasuk.idprodukm=produk.idproduk and month(tanggalmasuk)='$bln' group by idprodukm order by jumlah desc");
                            $data4 = mysql_fetch_array(mysql_query("SELECT tanggalmasuk,sum(jumlahmasuk) as jumlah FROM detbarangmasuk,barangmasuk where barangmasuk.idBMK=detbarangmasuk.kdbrgmsk and month(tanggalmasuk)='$bln'"));
                            $jmlmasuk = $data4['jumlah'];
                        ...
                        # code...
                        $sql=mysql_query("SELECT * from produk,transaksi,barangkeluar where produk.idproduk=barangkeluar.idprodk and barangkeluar.idBKK=transaksi.idBKeluar");
                        ...
                        $z=mysql_query("SELECT idproduk,namaproduk,sum(jumlahkeluar) as jml,count(idtransaksi) as kd,sum(totalbayar) as total,stok from produk,transaksi,barangkeluar where produk.idproduk=barangkeluar.idprodk and barangkeluar.idBKK=transaksi.idBKeluar group by idproduk order by jml desc");
                          $data2 = mysql_fetch_array($q);
                           $x=mysql_query("SELECT tanggalmasuk,idproduk,namaproduk,sum(jumlahmasuk) as jumlah,sum(hargaawal*jumlahmasuk) as totalbeli FROM detbarangmasuk,barangmasuk,produk WHERE detbarangmasuk.kdbrgmsk=barangmasuk.idBMK and detbarangmasuk.idprodukm=produk.idproduk group by idprodukm order by jumlah desc");
                            $data4 = mysql_fetch_array(mysql_query("SELECT sum(jumlahmasuk) as jumlah FROM detbarangmasuk"));
                            $jmlmasuk = $data4['jumlah'];
                      }
                      ...
                      while ($user=mysql_fetch_array($sql)) {
                        ...
                        echo "<td>".$no++."</td>";
                        echo "<td>".tgl($user['tanggaltransaksi'])."</td>";
                        echo "<td>".$user['idtransaksi']."</td>";
                        echo "<td>".$user['idproduk']."</td>";
                        echo "<td>".$user['namaproduk']."</td>";
                        echo "<td>".$user['jumlahkeluar']."</td>";
                        echo "<td>".rupiah($user['totalbayar'])."</td>";
                        ?>
...
<div class="btn-group float-right">
  ...
  <div class="dropdown-menu">
    <button type="button" class="btn-warning" data-toggle="modal" data-target="#brgmsk<?php echo $data2['tanggaltransaksi'] ?>">Detail Barang Masuk</button><br>
    <button type="button" class="btn-danger" data-toggle="modal" data-target="#updatedata<?php echo $data2['tanggaltransaksi'] ?>">Detail Barang Keluar</button><br>
  </div>
</div>  
<br>                 
                    ...
                    <?php }?>
                    <label>Jml Brg Masuk</label>
                    <input type='text' class='' readonly name='byr' id='byr' value="<?php echo $jmlmasuk;?>" >
           ...
           <div class="modal fade" id="updatedata<?php echo $data2['tanggaltransaksi'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel<?php echo $data2['tanggaltransaksi'] ?>" aria-hidden="true">
      ...
      <div class="form-group">
        <?php if (isset($_GET['bulan'])) {
                      # code...
                    ?>
                    <input type="text" class="form-control" style="font-size: 30px;background-color: yellow;" readonly name="nama" id="nama" placeholder="" value="<?php echo bulan($data2['tanggaltransaksi']);?>">
                    <?php }
                    else if (isset($_GET['tgl_upload'])) 
                    {  ?>
                    <input type="text" class="form-control" style="font-size: 30px;background-color: yellow;" readonly name="nama" id="nama" placeholder="" value="<?php echo tgl($_GET['tgl_upload']);?>">
                     <?php }  
                     else if (!isset($_GET['bulan']) or !isset($_GET['tgl_upload'])) {?>
                      <input type="text" class="form-control" style="font-size: 30px;background-color: yellow;" readonly name="nama" id="nama" placeholder="" value="ALL"><?php } ?>
      </div>
                ...
                <table class="table table-bordered" id="dataTables2" width="100%" cellspacing="0">
                  ...
                  <?php 
                  $no2=1;
                      while ($user=mysql_fetch_array($z)) {
                        //print_r($customer); untuk ngecek aja .. kalau udah muncul teruskan berikut //
  ...
  <div class="modal fade" id="brgmsk<?php echo $data2['tanggaltransaksi'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel<?php echo $data2['tanggaltransaksi'] ?>" aria-hidden="true">
                        <div class="modal-dialog modal-xl">
                          <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="form-group">
        <?php if (isset($_GET['bulan'])) {
                ...
                <table class="table table-bordered" id="dataTables2" width="100%" cellspacing="0">
                  ...
                  <?php 
                  $no2=1;
                      while ($user=mysql_fetch_array($x)) {
                        //print_r($customer); untuk ngecek aja .. kalau udah muncul teruskan berikut //
